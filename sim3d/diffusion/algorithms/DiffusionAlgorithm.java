		...
		for (int x = 1; x < iWidth; x++) {
			for (int y = 1; y < iHeight; y++) {
				for (int z = 1; z < iDepth; z++) {
					...
					//for each discrete gridspace iterate through the 
					// neighbouring gridspaces
					...
					for (int r = -1; r < 2; r++) {
						for (int s = -1; s < 2; s++) {
							for (int t = -1; t < 2; t++) {
								...
								// this space and the target space if there is no
								// difference in gradient then it won't diffuse
								double iDelta = (m_adDiffusionCoefficients[r + 1][s + 1][t + 1] 
										
										//find the diffusion coefficient, TODO why plus 1 (because it starts from -1) 
										//scale it by the difference in concentration
										* (ia3Concentrations[x][y][z] - ia3Concentrations[x+ r] 
												[y + s][z + t]));
