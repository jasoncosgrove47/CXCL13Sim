...
public class FDCGenerator {
	...
	public static int generateStroma3D(int iWidth, int iHeight, int iDepth,
		...
		//at each gridspace create an arraylist containing stromal cells
		...
		//reduce the cell counter
		int iRemainingCells = iCellCount - 1;

		//while there are cells to seed and the number of 
		// unbranched cells is greater than zero
		while (iRemainingCells > 0 && frclUnbranchedCells.size() > 0) {
			
			
			//pick the next cell, chooses closest to the origin
			...
			//TODO get simon to give a better explanation of this
			if (iRemainingCells == iCellCount - 1) { //this means that if this is the second cell
				iEdges++;
			}
	...
	protected static int createNewCells(int iWidth, int iHeight, int iDepth,
		...
		for (int i = 0; i < d3aDirections.length; i++) {
			...
			//add the new location to the origin location
			...
			//in this section we see if there are any other cells about and if so just
			//add a connection but otherwise, we add a new cell

			// Check if there already exists a cell within 1.6 grid spaces
			// If we don't do this, the grid is ridiculously dense
			
			//todo we can adjust this value to change the properties of the network
			...
			if (d3lAdjacent.size() > 1
				...
				//make sure you dont pick the origin
