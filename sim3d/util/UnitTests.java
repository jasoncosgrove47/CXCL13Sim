...
public class UnitTests extends AbstractAnalysis {
	...
	public void testRotateVectorToVector() {
		...
		for (int i = 2; i < lines; i++) {
			curPoint = Vector3DHelper.rotateVectorToVector(curPoint, d3Point1,
					d3Point2);
		...
		for (int i = 0; i < 100; i++) {
			...
			colors[i] = new Color(0, (i < 50) ? 255 : 0, (i < 50) ? 0 : 255,
					255);
		}

		for (int i = 100; i < c3dalPoints.size(); i++) {
			points[i] = c3dalPoints.get(i);
			colors[i] = new Color((int) (200 + (i - 100.0) / (lines * 50.0)
					* 55), 0, 0, 255);
		}
	...
	public void testVector3DHelperEqDistPointsOnSphere() {
		...
		for (int i = 0; i < size; i++) {
			points[i] = new Coord3d(ad3Points[i].x, ad3Points[i].y,
					ad3Points[i].z);
			colors[i] = new Color(0, 0, 0, 100);
		}
	...
	public void testParticleDiffusion() {
		...
		for (int x = 0; x < 125; x++) {
			for (int y = 0; y < 125; y++) {
				for (int z = 0; z < 125; z++) {
					distance += ((62 - x) * (62 - x) + (62 - y) * (62 - y) + (62 - z)
							* (62 - z))
							* p.field[x][y][z];
				}
			}
