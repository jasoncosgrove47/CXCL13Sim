...
public class StromaGenerator {
	...
	public static int generateStroma3D(int iWidth, int iHeight, int iDepth,
		...
		FRCCell frcInitialCell = new FRCCell(iWidth / 2.0, iHeight / 2.0,
				iDepth / 2.0);
		...
		while (iRemainingCells > 0 && frclUnbranchedCells.size() > 0) {
			FRCCell frcNextCell = pickNextCell(iWidth, iHeight, iDepth,
	 ...
	 */
	public static ArrayList<FRCCell> getAdjacentCells(int iWidth, int iHeight,
			int iDepth, ArrayList<FRCCell>[][][] frcla3CellLocations,
			FRCCell frcPoint, double iMaxDistance) {
	...
	protected static int createNewCells(int iWidth, int iHeight, int iDepth,
			ArrayList<FRCCell> frclCellLocations,
			ArrayList<FRCCell>[][][] frcla3CellLocations, FRCCell frcOrigin,
		...
		for (int i = 0; i < d3aDirections.length; i++) {
			...
			ArrayList<FRCCell> d3lAdjacent = getAdjacentCells(iWidth, iHeight,
			...
			if (d3lAdjacent.size() > 1
				...
				FRCCell newLoc = d3lAdjacent.get(Settings.RNG
	...
	protected static Double3D[] generateDirections(int iWidth, int iHeight,
			int iDepth, ArrayList<FRCCell>[][][] frcla3CellLocations,
			FRCCell frcLocation, int iCellCount) {
		...
		do {
			...
			for (int i = 1; i < iCellCount; i++) {
				...
				if (getAdjacentCells(iWidth, iHeight, iDepth,
						frcla3CellLocations,
						new FRCCell(frcLocation.d3Location.add(d3aReturn[i])),
						1.1).size() > 0) {
					i--;
					continue;
				}
			...
			if (getAdjacentCells(iWidth, iHeight, iDepth, frcla3CellLocations,
					new FRCCell(frcLocation.d3Location.add(d3aReturn[0])), 1.2)
					.size() > 0) {
				bFail = true;
				continue;
			}
	 ...
	 * @return
	 */
	protected static FRCCell pickNextCell(int iWidth, int iHeight, int iDepth,
			ArrayList<FRCCell> frclCellLocations,
		...
		FRCCell frcOrigin = new FRCCell(iWidth / 2.0, iHeight / 2.0,
				iDepth / 2.0);
